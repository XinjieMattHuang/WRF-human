...
nc_path = 'F:/OneDrive - connect.hku.hk/Xinjie Huang/6 WRF+HEATS/Work/WRF-HEATS codes/wrfout_10meters_horizontal_Hour55.nc' # Change to calculated PST result nc file
...
pst_10m = np.array(loadnc.variables['SLOPE'][0,:,:])
...
cs =  plt.imshow(imarray_10m_out, cmap='Greys', aspect='auto',origin='lower', alpha=0.5) # You can adjust the range of color bar with clim
cs2 =  plt.imshow(mask_LUM_land , cmap='jet', aspect='auto',origin='lower', interpolation='nearest')

cbar = plt.colorbar(cs2)
cbar.set_label('slope (°)',fontsize=12) # You may edit the description of color bar 
plt.xticks(np.arange(0,6375,1274),list_long,fontsize=12)
plt.yticks(np.arange(0,4800,959),list_lat,fontsize=12)
plt.xlabel('Longitude (°E)', fontsize=12)
plt.ylabel('Latitude (°N)', fontsize=12)

plt.savefig('F:/OneDrive - connect.hku.hk/Xinjie Huang/6 WRF+HEATS/Work/3. Results - Baseline, PFM, and slopes/3.3 Slope/Slope_map.png',dpi=2000) # Change to your directory and graphs output path
